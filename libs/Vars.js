...
loader.var('objInstances', /lowerBody\),\w+\|\|\w+\.(\w+)\./, 1),

loader.var('getWorldPosition', /var \w+=\w+\.camera\.(\w+)\(\);/, 1);

loader.patch('Skins', /((?:[a-zA-Z]+(?:\.|(?=\.skins)))+)\.skins(?!=)/g, (match, player) => `${loader.context.key}.skins(${player})`);
...
loader.patch('Controls', /\);\w+\.controls=(\w+);/, (match, controls) => `${match}${loader.context.key}.controls(${controls});`);
...
loader.patch('Timer', /(\w+\.exports)\.(kickTimer)=([\dex]+)/, (match, object, property, value) => `${loader.context.key}.timer(${object},"${property}",${value})`);
